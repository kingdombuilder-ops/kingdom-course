/* =============================================================================
   ...
   FAIL-OPEN + OBSERVABILITY: the callers in api/companion.js catch any
   throw from these functions and ALLOW the request (a KV outage must not
   take down the Companion). That trade is only safe if the fail-open is
   observable — otherwise a KV outage silently disables all rate limiting
   with no signal. CLOSED in Commit 8: both catch sites now capture to
   Sentry (area:ratelimit, limiter:ip|user) alongside the console.error,
   verified end-to-end on a preview deploy. See lib/companion-sentry.js.
